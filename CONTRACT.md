...
このファイルは「Scratch互換 音ゲーサブセット」の全モジュールが従う契約を定義する。
...
```
class ChartLoader {
  static validate(chart) -> { ok:bool, errors:[string] }  // chart.schema準拠
  static normalize(chart) -> chart   // notes を timeMs昇順安定ソート, 既定値補完
  static async load(url) -> chart    // fetch + validate + normalize
}
```
...
class JudgeSystem {
  ...
  // 有効オフセット: chartOffsetMs + userOffsetMs + deviceCalibMs
  setOffsets({chartOffsetMs, userOffsetMs, deviceCalibMs})
  effectiveOffsetMs()->number
  // 入力時刻(audio基準, ms)とノーツのtimeMsからhitErrorと判定
  judge(noteTimeMs, hitAudioMs) -> { result:"perfect"|"great"|"good"|"miss", hitErrorMs }
  classify(hitErrorMs) -> "perfect"|"great"|"good"|"miss"
  // miss閾値: |err|>goodMs。閾値は設定で可変。
}
```
...
```
class NoteSpawner {
  constructor(runtime, chart, { judgeY:-120, spawnY:180, laneX:[...], noteSpeed })
  // noteSpeed: y units/sec。spawnY→judgeYへ到達するleadTimeから逆算可
  update(songTimeMs)             // 出現すべきノーツをclone生成、活ノーツのy更新
  activeNotes -> [{id,lane,timeMs,clone,...}]   // 判定用配列（描画と分離）
  consume(note)                  // 判定済みノーツを除去
  laneXFor(lane)->number
}
```
...
class RhythmGame {
  ...
  async init()                   // runtime構築, sound preload, chart load
  start()                        // greenFlag相当, BGM schedule, tick開始
  // 入力→該当レーンの最近ノーツをjudge→スコア/コンボ更新→エフェクト
  onLaneHit(lane, hitAudioMs)
  songTimeMs()->number           // (audioContext.currentTime - bgmStart)*1000 - effectiveOffset
  state: { score, combo, maxCombo, counts:{perfect,great,good,miss} }
}
```
レーンキー既定: lane0=D, lane1=F, lane2=J, lane3=K。
...
class DebugOverlay {
  ...
  toggle()
  render(ctx)                    // FPS, clone数, audioClock vs perfClock, hitError散布図,
                                 // 直近イベントログ, list dump
  logEvent(text)
...
## 6. tools/（Node, ES Modules）

### validate-chart.js   `node tools/validate-chart.js <chart.json>` → 検証結果
...
class Sb3Generator {
  ...
  // DSL step/reporter → sb3 blocks（§7対応表）
}
```

---

## 7. sb3 opcode 対応表（DSL → Scratch VM opcode）
...
## 8. 譜面 chart.json（spec/chart.schema.json 準拠）
§ chart.schema.json を参照。判定既定 Perfect±40 / Great±80 / Good±120 / Miss超過。

## 9. 命名・テスト
- ファイル: engine/*.js, game/*.js は PascalCase。tools/*.js, tests/*.js は kebab/camel。
