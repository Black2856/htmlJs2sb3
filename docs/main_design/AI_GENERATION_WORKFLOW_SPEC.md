...
AIがユーザー要求を検証可能なDSLへ変換し、HTMLJS Runtimeで確認してSB3を生成する標準手順を定義する。Phase 7.1ではローカル作品を`workspace/<name>/`へ置くpreview/exportフローを提供するが、生成UIや新Runtime機能は実装しない。
...
- `workspace/<name>/project.ts`の作品DSL。
...
Phase 7のfixtureとSB3出力テストでは、素材を各作品の`assets/`
（`workspace/<name>/assets/`）に置く。共有プールは持たず、外部素材を無断で追加しない。
